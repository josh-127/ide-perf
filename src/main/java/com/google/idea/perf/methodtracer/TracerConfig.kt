...
object TracerConfig {
    ...
    fun trace(pattern: TracePattern, flags: Int, parameters: Collection<Int>) {
        when (pattern) {
            is TracePattern.Exact ->
                setTrace(pattern.method, flags, parameters)
            is TracePattern.ByMethodName ->
                setTrace(pattern.className, pattern.methodName, flags, parameters)
            is TracePattern.ByClassPattern -> {}
            is TracePattern.ByMethodPattern -> {}
        }
    }
    ...
    fun untrace(pattern: TracePattern) {
        when (pattern) {
            is TracePattern.Exact ->
                setTrace(pattern.method, 0, emptyList())
            is TracePattern.ByMethodName ->
                setTrace(pattern.className, pattern.methodName, 0, emptyList())
            is TracePattern.ByClassPattern -> {}
            is TracePattern.ByMethodPattern -> {}
        }
    }

    fun setTrace(method: Method, flags: Int, parameters: Collection<Int>) {
