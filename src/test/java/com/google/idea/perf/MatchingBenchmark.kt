...
private const val NUM_ITERATIONS = 250

private class BenchmarkResult(totalNanos: Long) {
...
private fun FuzzySearcher.benchmarkPattern(
    ...
    val resultSet = ArrayList<BenchmarkResult>(pattern.length)
    ...
    for (i in 1..pattern.length) {
        ...
        resultSet.add(BenchmarkResult(elapsedTime))
    }
...
fun main() {
    ...
    val results = ArrayList<List<BenchmarkResult>>()
    ...
    val largestResultSet = results.maxBy { it.size } ?: return
